...
export function setNavigation(ref: NavigationContainerRef<any>) { navigationRef = ref; }

function handleNotificationPress(notification: Notification) {
  const nome = notification.data?.nome ?? '';
  const dose = notification.data?.dose ?? '';
  const horario = notification.data?.horario ?? '';
  const usuarioTelefone = notification.data?.usuarioTelefone ?? '';

  if (navigationRef) navigationRef.navigate('ConfirmarConsumo', { nome, dose, horario, usuarioTelefone });
}

notifee.onForegroundEvent(({ type, detail }) => { if (type === EventType.PRESS && detail.notification) handleNotificationPress(detail.notification); });
...
export async function configurarAlarme(med: any) {
  ...
  await notifee.createChannel({ id:'medicamentos', name:'Medicamentos', importance:AndroidImportance.HIGH, sound:'default' });
  const now = new Date();
  const uniqueIdBase = nome;
  const scheduled = await notifee.getTriggerNotifications();
  for (const notif of scheduled) { const notifId = (notif.notification as any).id; if (notifId?.startsWith(uniqueIdBase)) await notifee.cancelNotification(notifId); }
  ...
  for (const horarioStr of horarios) {
    ...
    for (const diaKey in dias) {
      ...
      const trigger: TimestampTrigger = { type:TriggerType.TIMESTAMP, timestamp:triggerDate.getTime() };
      const notifId = `${uniqueIdBase}-${diaKey}-${hour}-${minute}`;
      try { await notifee.createTriggerNotification({
        id: notifId,
...
export async function agendarTodosMedicamentos(telefone: string) {
  ...
  } catch (err) { console.error('Erro ao agendar todos os medicamentos:', err); }
}
